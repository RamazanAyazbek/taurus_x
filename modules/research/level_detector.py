...
class LevelDetector:
  ...
  def find_raw_levels(self, df: pd.DataFrame, window: int = 5) -> list:
    """Поиск чистых разворотных пиков и впадин без микро-шума."""
    levels = []
  ...
  def merge_and_cluster_all_levels(
    ...
    """Укрупненная кластеризация для исключения линий каждые $300-$400."""
    if not raw_levels:
      return []

    # Волатильность базового диапазона (динамический полуразмер зоны)
    ...
    # Расширили порог объединения точек (2.0x width), чтобы сливать близкие линии в 1 массивную зону
    cluster_threshold = zone_half_width * 2.0
    ...
    for lvl in raw_levels:
      if not current_zone:
        ...
        if lvl["price"] - prev_price <= cluster_threshold:
          current_zone.append(lvl)
        else:
    ...
    for zone in zones:
      ...
      touches = len(zone)
      # Вес формируется строго: D1 дает сразу +5 баллов, каждый touch H1 дает +1.5 балла
      score = sum([x["weight"] for x in zone]) + int(touches * 1.5)

      tfs = [x["tf"] for x in zone]
      primary_tf = "D1" if "D1" in tfs else "H1"

      # Жесткий фильтр слабой пыли
      if score >= min_score:
  ...
  def get_tracked_zones(self, min_score: int = 5) -> dict:
    # 🎯 Увеличили охват: D1 = 180 дней (~6 месяцев), H1 = 360 свечей (15 дней)
    df_d1 = self.fetch_candles(interval="1d", limit=180)
    df_h1 = self.fetch_candles(interval="1h", limit=360)
    current_price = df_h1["close"].iloc[-1]

    # Окно поиска: D1 = 5 дней (крупные развороты), H1 = 4 часа
    raw_d1 = self.find_raw_levels(df_d1, window=5)
    raw_h1 = self.find_raw_levels(df_h1, window=4)

    all_raw = []
    for r in raw_d1:
      r["tf"] = "D1"
      r["weight"] = 5  # Высокий приоритет дневных уровней
      all_raw.append(r)
    ...
    # Смотрим зоны в пределах ±12% от текущей цены (чтобы видеть важные исторические блоки)
    reach_limit = 0.12
    filtered_zones = [
        z
        for z in zones
  ...
  def print_report(self):
    ...
    scan_range = cp * 0.02  # Сканирование реакций в пределах 2%
    near_supps = [z for z in supports if (cp - z["core_price"]) <= scan_range]
    near_resis = [
        z for z in resistances if (z["core_price"] - cp) <= scan_range
    ]
    ...
    print("-" * 90)
    print("💡 LEFEVRE PATH OF LEAST RESISTANCE (Intraday range 2.0%):")
